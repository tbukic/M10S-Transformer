"""Direct 10-digit training with larger models.

No curriculum — train directly on full random 10-digit addition.
Goal: Find the minimum d_model that can learn 10-digit addition.

This validates the architecture before attempting parameter minimization.
"""
...
def generate_batch(batch_size, device):
    """Generate full 10-digit addition batch on GPU."""
    ...
    a_t = torch.stack(a_digits, dim=1).flip(1)
    b_t = torch.stack(b_digits, dim=1).flip(1)
...
def evaluate(model, n_samples, device, seed=42):
    """Evaluate exact-match and per-digit accuracy on full 10-digit."""
    model.eval()
    correct = 0
    total = 0
...
def show_examples(model, device, n=5):
    ...
    for i in range(n):
        ...
        a_str = "".join(str(d) for d in seq[:MAX_DIGITS])
        b_str = "".join(str(d) for d in seq[MAX_DIGITS+1:MAX_DIGITS*2+1])
...
def train(d_model, n_repeats, epochs, lr, seed=0, use_wandb=False):
    ...
    config = TransformerConfig(
        d_model=d_model, n_heads=1, n_layers=1, d_ff=d_model,
    ...
    print(f"\n{'='*70}")
    print(f"Direct 10-digit: d_model={d_model}, repeats={n_repeats}, params={n_params}")
    ...
    if use_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"direct_d{d_model}_r{n_repeats}_{n_params}p_s{seed}",
                config={"d_model": d_model, "n_repeats": n_repeats, "n_params": n_params,
                        "lr": lr, "epochs": epochs, "seed": seed, "direct_10digit": True},
                tags=["direct_10digit"],
                reinit=True,
            )
        except Exception:
    ...
    for ep in range(epochs):
        ...
        if ep % 1000 == 0 and ep > 0:
            ...
            if acc > best_acc:
                ...
                best_epoch = ep
                print(f"  ** BEST: {acc:.4f} **")
                ckpt_dir = Path(f"checkpoints/direct_d{d_model}_r{n_repeats}_s{seed}")
    ...
    return {
        "d_model": d_model, "n_repeats": n_repeats, "n_params": n_params,
        "seed": seed, "best_acc": best_acc, "best_epoch": best_epoch,
...
def main():
    ...
    result = train(args.d_model, args.repeats, args.epochs, args.lr, args.seed, args.wandb)

    with open("experiments/direct_10digit_results.json", "w") as f:
        json.dump(result, f, indent=2)

    print(f"\nSaved results to experiments/direct_10digit_results.json")
