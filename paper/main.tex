...
We use three evaluation sets, generated with different RNG seeds and
mutually disjoint:

\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{Evaluation set} (seed=42): 10K and 50K pairs used for
    checkpoint selection (200-sample subset) and final accuracy reporting
    (Tables~\ref{tab:main},~\ref{tab:targeted}). Targeted fine-tuning
    (Section~\ref{sec:targeted}) draws error pairs from this set.
  \item \textbf{Holdout set} (seed=123 for 10K, seed=99 for 50K): never
    used during training, checkpoint selection, or targeting. Validates
    that targeted models genuinely generalize
    (Table~\ref{tab:holdout}).
  \item \textbf{Verification set} (seed=2025): the official AdderBoard
    \texttt{verify.py} test of 10,010 pairs. External to our pipeline
    (Table~\ref{tab:verify}).
\end{itemize}

Training data is generated on-the-fly from a per-run seeded RNG. The
input space is $[0, 10^{10}-1]^2 = 10^{20}$ pairs. With $\sim$6.4M
training samples (50K steps $\times$ 128 batch), the expected overlap
with any 10K set is $10^4 \times 6.4 \times 10^6 / 10^{20} \approx
6 \times 10^{-10}$---effectively zero.

\textbf{Checkpoint selection.} During training, we evaluate on 200
samples from the evaluation set to select the best checkpoint. This
introduces a mild dependence on the evaluation distribution, which is
standard practice in grokking research. We note this as a methodological
limitation and validate all key results on the holdout set.
...
\begin{table}[t]
\centering
\caption{Main results on the evaluation set. ``Natural''
models are trained without any evaluation-set-derived information in the
...
50K evaluation-set samples through a natural 4-stage fine-tuning pipeline
with no intervention beyond checkpoint selection.

\textbf{The 83p and 86p results use targeted fine-tuning} (Section~\ref{sec:targeted}),
which includes evaluation-set error pairs in training data. However,
all targeted models maintain their accuracy on the holdout set
(Table~\ref{tab:holdout}).

\begin{table}[t]
\centering
\caption{Results on the holdout set (10K: seed=123, 50K: seed=99),
disjoint from training and the evaluation set.}
\label{tab:holdout}
\vspace{4pt}
\small
\begin{tabular}{@{}rlcc@{}}
...
error pairs identified from the evaluation set as additional training
data. The model is directly trained on inputs it previously predicted
incorrectly.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item Evaluate the model on the evaluation set.
...
converge in a single round. All targeted models achieve 100\% on the
evaluation set.}
\label{tab:targeted}
\vspace{4pt}
...
Inspired by staghado's concurrent work on the AdderBoard showing that AdamW
...
All our models achieve 10,010/10,010 correct on the official
\texttt{verify.py} (Table~\ref{tab:verify}). Hand-coded models
(\eg, 20-parameter constructive solutions) are excluded.}
...
\midrule
\multicolumn{5}{l}{\textit{Official leaderboard (accepted)}} \\
...
\midrule
\multicolumn{5}{l}{\textit{Pending submissions (not yet accepted)}} \\
67  & evindor   & 100\%     & 1L, d=5, 1h, ff=2  & Circular embed, rank-1 out \\
\textbf{83}  & \textbf{Ours} & \textbf{100\%}$^\dagger$ & \textbf{Qwen3, d=3, 1h, ff=2} & \textbf{Iterated targeted} \\
\textbf{86}  & \textbf{Ours} & \textbf{100\%}$^\dagger$ & \textbf{Qwen3, d=3, 1h, ff=2} & \textbf{L-BFGS + targeted} \\
\textbf{89}  & \textbf{Ours} & \textbf{100\%} & \textbf{Qwen3, d=3, 1h, ff=2} & \textbf{4-stage natural FT} \\
\textbf{101} & \textbf{Ours} & \textbf{100\%}$^\dagger$ & \textbf{Qwen3, d=3, 1h, ff=2} & \textbf{Targeted (2 pairs)} \\
\textbf{122} & \textbf{Ours} & \textbf{100\%} & \textbf{Qwen3, d=3, 1h, ff=3} & \textbf{Cosine + grokking} \\
122 & staghado   & 99.95\%    & Qwen3, d=3, 1h, ff=3 & AdamW + L-BFGS \\
162 & fblissjr  & 100\%     & 1L, d=3, 3h, ff=6  & Fixed attn mask (hybrid) \\
262 & lichengliu03 & 99.95\% & 1L, d=4, 1h, ff=6 & Rank-3, curriculum \\
275 & ryanyord  & 99.98\%   & 1L, d=4, 1h, ff=8  & SVD truncation of 311p \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize $^\dagger$Uses targeted fine-tuning with evaluation-set error pairs (Section~\ref{sec:targeted}).
All ``Ours'' results verified at 100.00\% on official \texttt{verify.py} (10,010/10,010).}
\end{table}

Notable concurrent work includes: a pending 67-parameter model
(MicroAdder) using parametric circular embeddings, rank-1 attention
output, and carry-heavy curriculum---currently the only trained model
smaller than ours; and hand-coded models reaching as low as 20 parameters
with constructive weight assignment.
Our base architecture builds on staghado's 122-parameter Qwen3 model
(d=3, 1h, hd=4, ff=3, RoPE $\theta$=3), which reaches 99.95\% accuracy.
Our contributions are the parameter compression chain
(tieKV, tieQO, shared norms: $122 \to 83$ parameters) and targeted
fine-tuning for 100\% accuracy. Their L-BFGS fine-tuning insight---that
AdamW converges to saddle points which second-order methods can
escape---also directly inspired our L-BFGS experiments
...
200-sample evaluations from the evaluation set during training. This
creates a dependence between checkpoint selection and final evaluation
on the same set. Results on the holdout set
(Table~\ref{tab:holdout}) confirm that selection bias is negligible:
89p s11127 achieves 0 errors on holdout 50K, and all targeted models
also achieve 0 errors.

\textbf{Targeted fine-tuning.} The 83p and 86p 100\% results use
evaluation-set errors as training data. However, these models achieve
0 errors on the holdout set (Table~\ref{tab:holdout}), confirming
generalization rather than memorization of targeted pairs. The
89p s11127 natural result remains our primary claim for unassisted
100\% accuracy.
