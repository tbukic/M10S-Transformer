"""AdderBoard submission: M10S-86p (86 parameters).

1-layer decoder-only transformer, d=3, 1h/1kv, hd=4, ff=2, RoPE theta=3, SwiGLU.
Tied K=V, Tied O=Q^T, shared block RMSNorms, L-BFGS + targeted fine-tuning.
"""
...
_CHECKPOINT = _PROJECT_ROOT / "checkpoints" / "qwen3_d3_ff2_86p_tiekv_tieqo_shbnorm_s1_targeted" / "best.pt"
...
METADATA = {
    ...
    "tricks": [
        ...
        "Tied K=V",
        "Tied O=Q^T",
        "Shared block RMSNorms",
        "RoPE (zero params)",
        "QK norms",
        "L-BFGS + targeted fine-tuning",
    ],
}
