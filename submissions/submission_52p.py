"""AdderBoard submission: M10S-52p (52 parameters).

1-layer decoder-only transformer with circular arc embedding + K=rotation(Q) + gate=alpha*up.
d=3, 1h/1kv, hd=4, ff=2, RoPE theta=3, SwiGLU, shared norms, tied Q=O projections.
"""
...
# Need to import obsolete module for KrotQ build
...
_CHECKPOINT = _PROJECT_ROOT / "checkpoints" / "qwen3_arc_52p_KrotQ_targeted" / "best.pt"

METADATA = {
    "name": "M10S-52p",
    "author": "Tom Bukic",
    "params": 52,
    "architecture": "1L decoder-only transformer + circular arc embed, d=3, 1h/1kv, hd=4, ff=2, K=rot(Q), gate=a*up, tieQO, shnorm",
    "tricks": [
        "Circular arc embedding (3 params instead of 30)",
        "K = rotation(Q) (1-param rotation replaces 12-param key projection)",
        ...
        "RoPE (zero params)",
        "QK norms",
        "Grokfast-EMA (alpha=0.98, lambda=1.0)",
        "Targeted fine-tuning (2 iterations, 14 cumulative error pairs)",
    ],
}
...
def build_model():
    ...
    cfg_name = "52p_ff2_tieQO_KrotQ_gateA_shnorm"
