"""AdderBoard submission: M10S-45p (45 parameters).
...
All 3 RMSNorms shared, tied Q=O projections.
"""
...
_CHECKPOINT = _PROJECT_ROOT / "checkpoints" / "qwen3_arc_45p_KrotQ_targeted" / "best.pt"

METADATA = {
    "name": "M10S-45p",
    "author": "Tom Bukic",
    "params": 45,
    "architecture": "1L decoder-only transformer + circular arc embed, d=3, 1h/1kv, hd=4, ff=2, K=rot(Q), V=Q, tieQO, shnorm",
    "tricks": [
        "Circular arc embedding (3 params instead of 30)",
        "K = rotation(Q) (1-param 2D rotation replaces 12-param key projection)",
        "V = Q (value projection shares query weights, saves 12 params)",
        "Tied Q=O projections (output = Q transposed)",
        "All 3 RMSNorms shared (ln1=ln2=final_norm, saves 6 params)",
        "RoPE (zero params)",
        "QK norms",
        "Curriculum + Grokfast-EMA + targeted fine-tuning",
    ],
}
...
def build_model():
    ...
    cfg_name = "45p_ff2_tieQO_KrotQ_shnorm_VeqQ"
